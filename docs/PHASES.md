...
**Exit criteria:**
- [ ] A PR with a one-line change shows all checks green: `lint`, `typecheck`, `unit`, `e2e`, `build`, `flow-check`, `pr-title`, plus the Vercel preview check.
- [ ] The preview URL renders the placeholder page.
- [ ] Merging deploys to `https://goal-tracker-bus321.vercel.app`.
- [ ] Confirmed whether Neon created a preview branch for the PR (Neon console → Branches, or the deployment's connected-resource panel); result recorded in `ARCHITECTURE.md § Environments`.
- [ ] Dependabot opened (or is scheduled to open) its first PR and it is auto-labeled `skip-flow-check`.
- [ ] `docs/PHASES.md` status updated.
