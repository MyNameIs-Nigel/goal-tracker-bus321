...
Partner: create today's check-in, edit today's note. Owner and viewer: read.
...
- **And** a direct check-in for another date or another user id is rejected (ROLE-04)

### PCI-06 Owner and viewers don't get the button
- **Given** the owner or a viewer on `/today`
- **Then** the section is read-only; a direct check-in action is rejected
...
Section at the bottom of the day page: one row per partner (avatar, name, status/time, note in muted text). For the current partner on today's page, the row expands with the button or the note field. Time is shown in Denver time with the `AM/PM` format; no seconds.
